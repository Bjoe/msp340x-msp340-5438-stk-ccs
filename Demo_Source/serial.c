...
xComPortHandle xSerialPortInitMinimal( unsigned portLONG ulWantedBaud, unsigned portBASE_TYPE uxQueueLength )
{
	...
	unsigned portLONG ulBaudRateCount = configLFXT_CLOCK2_HZ / ulWantedBaud;
	...
	portENTER_CRITICAL();
	{
		...
		/* Use SMCLK. */
		UCA3CTL1 = UCSSEL0 | UCSSEL1;
		
		/* Setup baud rate low byte. */
		UCA3BR0 = ( unsigned portCHAR ) ( ulBaudRateCount & ( unsigned long ) 0xff );

		/* Setup baud rate high byte. */
		ulBaudRateCount >>= 8UL;
		UCA3BR1 = ( unsigned portCHAR ) ( ulBaudRateCount & ( unsigned long ) 0xff );
...
signed portBASE_TYPE xSerialPutChar( xComPortHandle pxPort, signed portCHAR cOutChar, portTickType xBlockTime )
{
	...
	completed and switched itself off. */
        signed portBASE_TYPE xReturn = xQueueSend( xCharsForTx, &cOutChar, xBlockTime );
	UCA3IE |= UCTXIE;
...
interrupt void prvUSCI_A3_ISR( void )
{
signed char cChar;
